defmodule EcoServiceWeb.EcoServiceLive.Dashboard do
  ...
  def mount(_params, _session, socket) do
    ...
    top_5_community_produce_waste =
      top_5_community_and_waste
      |> Enum.map(fn details ->
        EcoServiceContext.get_community_by_id(details.community_id).name
      end)
    ...
    waste_details =
      top_5_community_and_waste
      |> Enum.map(fn top_community_detail ->
        EcoServiceContext.get_waste_by_community_id(top_community_detail.community_id)
      end)
    ...
    {:ok,
     ...
     |> assign(
       :highest_count_of_waste_produced_by_communities,
       Jason.encode!(highest_count_of_waste_produced_by_communities)
     )
  ...
  def handle_event("type_of_chart_change", params, socket) do
    type_of_chart =
      if params["type_of_chart"] == "total_waste" do
        "Total waste"
      else
        "Categories of Waste"
      end
